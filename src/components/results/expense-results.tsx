  ...
  return (
    <div className="flex flex-col space-y-4 border-t p-4 sm:p-6">
      <div className="text-muted-foreground flex items-start gap-2 text-sm">
        ...
        <span>
          Assumes {depositPercentage}% deposit and {interestRate}% variable interest rate
        </span>
      </div>
      ...
      <div className="grid w-full grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        {monthlyRentalIncome > 0 && (
          <>
            <div className="space-y-1">
              <p className="text-muted-foreground text-sm font-medium">Monthly Rental Income</p>
              <p className="text-2xl font-bold text-green-600">${Math.round(monthlyRentalIncome).toLocaleString()}</p>
            </div>
            <div className="space-y-1">
              <p className="text-muted-foreground text-sm font-medium">Net Monthly Expenses</p>
              <p className="text-2xl font-bold">${Math.round(netMonthlyExpenses).toLocaleString()}</p>
            </div>
          </>
        )}
      </div>
